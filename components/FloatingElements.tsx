    ...
    <>
      ...
      <div className="currency-switcher">
        {['AED', 'USD', 'EUR', 'GBP'].map(c => (
          <button key={c} className={`currency-btn ${currency === c ? 'active' : ''}`} onClick={() => setCurrency(c as any)}>{c}</button>
        ))}
      </div>
      ...
      <button className="chat-trigger" onClick={() => setChatOpen(!chatOpen)} title="Chat with Rox">ROX</button>
